...
class TreeDialogController (object):
	def __init__(self, root_node, allow_multi=False, async_mode=False):
		...
		self.edit_btn = ui.ButtonItem(title='Edit', action=self.edit_action)
		if self.allow_multi:
			self.view.right_button_items = [self.edit_btn, self.done_btn]
	...
	def done_action(self, sender):
		...
		self.view.close()
	def edit_action(self, sender):
		self.table_view.allows_multiple_selection_during_editing=True
		self.table_view.allows_selection_during_editing=True
		self.table_view.allows_selection=True
		self.table_view.allows_multiple_selection=True
		self.table_view.set_editing(not self.table_view.editing)
...
def main():
	py_files = file_picker_dialog('Pick some .py files', multiple=True, select_dirs=True, file_pattern=r'^.*\.py$')
